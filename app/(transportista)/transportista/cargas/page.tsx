...
export default async function TransportistasCargasPage({
  ...
  const yaPostuladoEn = new Set(misPostulaciones.map((p) => p.cargaId));
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link href="/transportista/postulaciones" className="text-sm text-gray-600 hover:text-gray-800">
            Mis postulaciones
          </Link>
          <form action={logout}>
            <button
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-800">Cargas disponibles</h1>
        ...
        {success === "1" && (
          <div className="mb-6 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
            <p className="text-sm text-green-800 font-medium">¡Postulación enviada! La empresa te contactará si te selecciona.</p>
          </div>
        )}
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {carga.fechaCarga.toLocaleDateString("es-AR")} · {TIPO_LABELS[carga.tipoCarga] ?? carga.tipoCarga}
                    {carga.peso !== null && ` · ${carga.peso}t`}
                    {carga.presupuesto !== null && ` · $${carga.presupuesto.toLocaleString("es-AR")}`}
                  </p>
                </div>
                <span className="flex-shrink-0 text-sm text-green-700 font-medium">Ver →</span>
              </Link>
            ))}
          </div>
        )}
